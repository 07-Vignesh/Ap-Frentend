...
const handleAddFish = async (e) => {
  e.preventDefault();
  try {
    const response = await axios.post(' https://ap-backend-9neb.onrender.com/api/fishes',newFish);
    setItems(prev => [...prev, response.data]); // add new bird to list
    setAlertType('success');
    setAlertMessage('Fish added successfully!');
    setShowAddForm(false);
    setNewFish({ title: '', image: '', rate: '' });
    setTimeout(() => setAlertMessage(''), 3000);
  } catch (err) {
    console.error('Error adding fish:', err);
    setAlertType('error');
    setAlertMessage('Failed to add fish.');
    setTimeout(() => setAlertMessage(''), 3000);
  }
};
...
const { user } = useUser();
  ...
  try {
    await axios.delete(` https://ap-backend-9neb.onrender.com/api/fishes/${id}`);
    setItems(items.filter(item => item._id !== id)); 
    setAlertType('success');
    setAlertMessage('Fish item deleted successfully!');
    setTimeout(() => setAlertMessage(''), 3000);
  } catch (error) {
    console.error('Delete failed:', error);
    setAlertType('error');
    setAlertMessage('Failed to delete FIsh item.');
    setTimeout(() => setAlertMessage(''), 3000);
  }
};
...
const { isSignedIn } =  useAuth();
 ...
 useEffect(() => {
  const fetchItems = async () => {
    try {
      const res = await axios.get(`https://ap-backend-9neb.onrender.com/api/fishes`, {
...
{showAddForm && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <form onSubmit={handleAddFish} className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
      <h2 className="text-xl font-bold mb-4 text-black">Add New Fish</h2>
