...
const handleAddBird = async (e) => {
  e.preventDefault();
  try {
    const response = await axios.post('https://ap-backend-9neb.onrender.com/api/birds', newBird);
    setItems(prev => [...prev, response.data]);
    setAlertType('success');
    setAlertMessage('Bird added successfully!');
    setShowAddForm(false);
    setNewBird({ title: '', image: '', rate: '' });
    setTimeout(() => setAlertMessage(''), 3000);
  } catch (err) {
    console.error('Error adding bird:', err);
    setAlertType('error');
    setAlertMessage('Failed to add bird.');
    setTimeout(() => setAlertMessage(''), 3000);
  }
};
  ...
  try {
    await axios.delete(`https://ap-backend-9neb.onrender.com/api/birds/${id}`);
...
const { isSignedIn } =  useAuth();
 ...
 useEffect(() => {
  const fetchItems = async () => {
    try {
      const res = await axios.get(`https://ap-backend-9neb.onrender.com/api/birds`, {
...
{showAddForm && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <form onSubmit={handleAddBird} className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
      <h2 className="text-xl font-bold mb-4 text-black">Add New Bird</h2>
