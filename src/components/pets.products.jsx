...
const handleDelete = async (id) => {
  try {
    await axios.delete(`https://ap-backend-9neb.onrender.com/api/pets/${id}`);
...
const handleAddPet = async (e) => {
  e.preventDefault();
  try {
    const response = await axios.post('https://ap-backend-9neb.onrender.com/api/pets', newPet);
    setItems(prev => [...prev, response.data]); // add new bird to list
    setAlertType('success');
    setAlertMessage('Pets added successfully!');
    setShowAddForm(false);
    setNewPet({ title: '', image: '', rate: '' });
    setTimeout(() => setAlertMessage(''), 3000);
  } catch (err) {
    console.error('Error adding pets:', err);
    setAlertType('error');
    setAlertMessage('Failed to add pets.');
    setTimeout(() => setAlertMessage(''), 3000);
  }
};
...
const { isSignedIn } =  useAuth();
 ...
 useEffect(() => {
  const fetchItems = async () => {
    try {
      const res = await axios.get(`https://ap-backend-9neb.onrender.com/api/pets`, {
...
{showAddForm && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <form onSubmit={handleAddPet} className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
      <h2 className="text-xl font-bold mb-4 text-black">Add New Pets</h2>
