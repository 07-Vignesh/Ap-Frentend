...
const handleAddProduct = async (e) => {
  e.preventDefault();
  try {
    const response = await axios.post('https://ap-backend-9neb.onrender.com/api/products', newProduct);
...
const handleDelete = async (id) => {
  try {
    await axios.delete(`https://ap-backend-9neb.onrender.com/api/products/${id}`);
...
const { isSignedIn } =  useAuth();
 ...
 useEffect(() => {
  const fetchItems = async () => {
    try {
      const res = await axios.get(`https://ap-backend-9neb.onrender.com/api/products`, {
